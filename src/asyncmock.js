  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      if (!categoria) {
        ...
        resolve(
          MisProdutos.filter(
            (categoriaItem) => categoriaItem.Categoria === categoria
          )
        );
      }
    }, 500);
...
export const getCategorias = () => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const categorias = [...new Set(MisProdutos.map((p) => p.Categoria))];
      resolve(categorias);
    }, 500);
  });
};
